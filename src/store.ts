...
export const PRESET_COLORS = [
  "255,255,255", "209,213,219", "156,163,175", "100,116,139",
  "59,130,246",  "99,102,241",  "168,85,247",  "236,72,153",
  "239,68,68",   "249,115,22",  "245,158,11",  "234,179,8",
  "16,185,129",  "20,184,166",  "6,182,212",   "14,165,233",
  "132,204,22",  "251,191,36",
];
...
export const useTodoStore = create<State>((set, get) => ({
  ...
  setShowCreatedAt: async (id, show) => {
    const db = await getDb();
  ...
  setSubtasks: async (id, subtasks) => {
    const db = await getDb();
    await db.execute("UPDATE todos SET subtasks = ? WHERE id = ?", [JSON.stringify(subtasks), id]);
    set((s) => ({ todos: s.todos.map((t) => t.id === id ? { ...t, subtasks } : t) }));
  },
